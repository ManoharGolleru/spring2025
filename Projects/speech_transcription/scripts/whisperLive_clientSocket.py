...
class Client:
    ...
    def __init__(
        ...
        max_clients=4,
        max_connection_time=3600,   # 3600 seconds = 1 hour
        sio=None
    ):
        """
        ...
        # self.client_socket.keep_running = False
        print('all done')
        '''
    ...
    def process_segments(self, segments):
        ...
        msg = {'completed': [], 'incomp_text': ''}
        '''
        transcrResult[] completed
			string comp_time_start  # H:MM:SS
			string comp_time_end    # H:MM:SS
			string comp_text
		string incomp_text
		'''
        ...
        for i, seg in enumerate(segments):
            if not text or text[-1] != seg["text"]:
                ...
                if i == len(segments) - 1 and not seg.get("completed", False):
                    ...
                    print("IN PROCESS", seg)
                    msg['incomp_text'] = seg["text"]
                elif (self.server_backend == "faster_whisper" and seg.get("completed", False) and
                      (not self.transcript or
                        float(seg['start']) >= float(self.transcript[-1]['end']))):
                    self.transcript.append(seg)
                    print("DONE", seg)
                    # FIXME -- Convert 'start' and 'end' to H:MM:SS
                    msg['completed'].append({'comp_time_start': str(seg['start']), 'comp_time_end': str(seg['end']), 'comp_text': seg['text']})
        ...
        self.sio.emit('live_transcript', msg)
        
        # FIXME -- Uncomment stuff, or get rid of this option
...
class TranscriptionTeeClient:
    ...
    """
    def __init__(self, clients, save_output_recording=False, output_recording_filename="./output_recording.wav", deviceID=None):
...
class TranscriptionClient(TranscriptionTeeClient):
    ...
    def __init__(
        ...
        self.client = Client(
            ...
            max_connection_time=max_connection_time, sio=sio
        )
